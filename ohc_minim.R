...
for (e in 1:ne){
 for (l in 1:nl){
   ex<-nen[e]
   lv<-ll[l]
   ff<-sprintf("grep/ohc_%s_0-%d_%s.dat",ex,lv,lat_bnd)
   print(ff)
...
#  steric.cm<-as.matrix(read.table("steric_cmems.dat"))

ny<-2018-1993+1

steric.cm<-array(dim=c(ny,ne))
for (i in 1:ne){
   fscm<-mylist(paste("grep/steric_",nen[i],"_0-6000_",lat_bnd,".dat",sep=""))
   tmp<-read.table(fscm)
...
datin<-as.matrix(read.table("/data/andrea/sealev/altim/steric_tot.dat"))
obs<-datin[,2]
obs<-obs-mean(obs)
...
# OHC data
o_y<-2003:2018
tmp<-get_fld("GCOS_all_heat_content_2003-2018_ZJ_v22062020.nc","ohc_0-700m")
o_7<-anom(tmp)
e_7<-get_fld("GCOS_all_heat_content_2003-2018_ZJ_v22062020.nc","ohc_0-700m_uncertainty")
range(e_7)
e_7<-sqrt(e_7^2+2^2)
range(e_7)
tmp<-get_fld("GCOS_all_heat_content_2003-2018_ZJ_v22062020.nc","ohc_700-2000m")
o_2<-anom(tmp)
e_2<-get_fld("GCOS_all_heat_content_2003-2018_ZJ_v22062020.nc","ohc_700-2000m_uncertainty")
range(e_2)
e_2<-sqrt(e_2^2+0.3^2)
range(e_2)

tmp<-get_fld("GCOS_all_heat_content_2003-2018_ZJ_v22062020.nc","ohc_below_2000m")
ohcd_kvs<-anom(tmp)
tmp<-get_fld("GCOS_all_heat_content_2003-2018_ZJ_v22062020.nc","ohc_below_2000m_uncertainty")
ohcd_kvse<-tmp
...
cer<-cumsum(get_fld("CERES_TOA_2003-2018_ym.nc","gtoa_net_all_mon")*1.1)
mean(get_fld("CERES_TOA_2003-2018_ym.nc","gtoa_net_all_mon")*1.0)
cconv<-0.07169398
beta<-0.90
